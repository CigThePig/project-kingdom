...
export const EXPANSION_WAVE_2_CRISES_RELIGIOUS_TEXT: Record<string, EventTextEntry> = {
  ...
  evt_exp_w2_heretical_sermon: {
    ...
    body: 'A young preacher is drawing larger and larger crowds at the cathedral square with sermons the primate calls outright heresy. He denies no doctrine out loud — he merely asks questions in public that the faithful used to keep quiet. The {class_plural} await the crown\'s reply.',
    choices: {
  ...
  evt_exp_w2_saints_succession: {
    ...
    body: 'The old primate is dying, and the conclave has assembled to choose his heir. Each candidate carries a faction: one the Crown\'s creature, one the conclave\'s favourite, one a quiet reformer. The choice will echo for a generation. The {class_plural} await the crown\'s reply.',
    choices: {
  ...
  evt_exp_w2_witch_trial: {
    ...
    body: 'Three women in a border village have been accused of consorting with evil. The local magistrate has called for a formal trial; the commons want a bonfire by tomorrow; the clergy want it handled by diocesan court. You are asked to rule. The {class_plural} await the crown\'s reply.',
    choices: {
