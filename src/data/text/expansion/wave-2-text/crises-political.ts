...
export const EXPANSION_WAVE_2_CRISES_POLITICAL_TEXT: Record<string, EventTextEntry> = {
  ...
  evt_exp_w2_dynastic_challenge: {
    ...
    body: 'A distant cousin, long thought safely landless abroad, has appeared in the marches of {region} with a modest retinue and a claim to the throne. The claim is thin in law but thick in sympathy among the border lords. Left unanswered it will only grow.',
    choices: {
  ...
  evt_exp_w2_foreign_assassination: {
    ...
    body: 'A prince of {neighbor_short} has been killed — by a knife, in his own hall, by hands not yet named. The court of that realm demands public mourning or public answers. Your own spymaster is studiously looking elsewhere.',
    choices: {
  ...
  evt_exp_w2_bandit_lord_uprising: {
    ...
    body: 'In the hill country of {region} a hedge-knight has gathered a small army from discontented tenants and deserters. He styles himself a champion of the commons and is said to have taken three manor houses already. The march captains await orders.',
    choices: {
