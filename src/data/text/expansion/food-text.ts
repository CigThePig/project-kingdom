...
export const EXPANSION_FOOD_TEXT: Record<string, EventTextEntry> = {
  ...
  evt_exp_fod_fishing_fleet: {
    ...
    body: 'A delegation of coastal fishermen petitions the throne for funding to commission additional vessels. The {season} sea teems with catch they cannot harvest for want of boats, yet the cost of timber and rope would draw from a treasury in {treasury_tier} condition — with stores only at a {stores_tier} mark, {chancellor_or_fallback} counsels care. The {class_plural} await the crown\'s reply.',
    choices: {
  ...
  evt_exp_fod_distribution_inequity: {
    ...
    body: '{chancellor_or_fallback} warns that with stores at a {stores_tier} mark, the disparity has grown too visible to ignore: while noble tables groan under the weight of roasted meats and fresh bread, commonfolk queue for hours to receive meagre portions of thin gruel. Unrest simmers in the lower quarters of the capital.',
    choices: {
  ...
  evt_exp_fod_noble_hoarding: {
    ...
    body: '{spymaster_or_fallback} confirms that several noble houses have been quietly stockpiling grain and salted meat within their private holdings while the common granaries sit at a {stores_tier} mark. The lords claim they merely exercise prudent management, but the people see only greed in a time of want. The {class_plural} await the crown\'s reply.',
    choices: {
