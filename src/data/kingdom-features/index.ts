...
export const KINGDOM_FEATURE_REGISTRY: Record<string, KingdomFeatureDefinition> = {
  ...
  // ============================================================
  // Phase 9 Wave 2 — Critical crisis resolution features
  // Per CARD_AUDIT_RULES §2 severity-scaling corollary: Critical crises
  // must establish a feature, change policy, activate/resolve a storyline,
  // or trigger a bond-level diplomatic change on at least one branch.
  // Each branch below carries its own ongoing-effect feature that persists
  // after the crisis resolves, so the "Critical" tier is honest.
  // ============================================================
  'event:evt_exp_chain_border_war_mobilization:defensive_posture': {
    featureId: 'feature_defensive_posture',
    title: 'Defensive Posture',
    description: 'The marches stand on watch; forts are manned and patrols doubled.',
    category: 'military',
    ongoingEffect: { militaryReadinessDelta: 1, treasuryDelta: -2 },
  },
  'event:evt_exp_chain_border_war_mobilization:full_mobilization': {
    featureId: 'feature_full_mobilization',
    title: 'Full War Mobilization',
    description: 'Every levy called. The kingdom is a standing army until peace is won.',
    category: 'military',
    ongoingEffect: { militaryForceSizeDelta: 1, treasuryDelta: -3, commonerSatDelta: -1 },
  },
  'event:evt_exp_chain_drought_autumn:emergency_food_imports': {
    featureId: 'feature_grain_import_dependency',
    title: 'Foreign Grain Dependence',
    description: 'Convoys from abroad keep the granaries from emptying; the treasury pays for every sack.',
    category: 'economic',
    ongoingEffect: { foodDelta: 1, treasuryDelta: -3 },
  },
  'event:evt_exp_chain_drought_autumn:strict_rationing': {
    featureId: 'feature_drought_rationing_regime',
    title: 'Strict Rationing Regime',
    description: 'Quartermasters measure every loaf. Hunger eases; resentment does not.',
    category: 'infrastructure',
    ongoingEffect: { foodDelta: 1, commonerSatDelta: -1 },
  },
  'event:evt_exp_chain_spy_war_resolution:cut_losses_and_rebuild': {
    featureId: 'feature_rebuilt_spy_network',
    title: 'Rebuilt Spy Network',
    description: 'The old network is burned; new cells learn from its mistakes.',
    category: 'military',
    ongoingEffect: { espionageNetworkDelta: 1 },
  },
  'event:evt_exp_dip_trade_embargo_threat:accept_the_embargo': {
    featureId: 'feature_embargo_adaptation',
    title: 'Embargo Adaptation',
    description: 'The realm learns to live without foreign markets; domestic guilds expand into the gap.',
    category: 'economic',
    ongoingEffect: { merchantSatDelta: -1, commonerSatDelta: 1 },
  },
  'event:evt_exp_eco_debt_crisis:impose_austerity_measures': {
    featureId: 'feature_austerity_regime',
    title: 'Austerity Regime',
    description: 'Crown spending is slashed across every ministry. Debts recede; every class feels the squeeze.',
    category: 'economic',
    ongoingEffect: { treasuryDelta: 2, commonerSatDelta: -1, nobilitySatDelta: -1 },
  },
  'event:evt_exp_eco_debt_crisis:negotiate_with_creditors': {
    featureId: 'feature_creditor_accord',
    title: 'Creditor Accord',
    description: 'Restructured terms stretch the debt across generations; interest gnaws at the treasury each turn.',
    category: 'economic',
    ongoingEffect: { treasuryDelta: -1, merchantSatDelta: 1 },
  },
  'event:evt_exp_env_volcanic_ash_cloud:seal_granaries_and_ration': {
    featureId: 'feature_sealed_granaries',
    title: 'Sealed Granaries',
    description: 'Ash-proofed stores keep a portion of the harvest intact through every dark season after.',
    category: 'economic',
    ongoingEffect: { foodDelta: 1 },
  },
  'event:evt_exp_esp_enemy_infiltration:martial_law_purge': {
    featureId: 'feature_martial_law',
    title: 'Martial Law',
    description: 'Garrisons hold the city squares; curfew and papers-of-transit are the new normal.',
    category: 'infrastructure',
    ongoingEffect: { stabilityDelta: 1, commonerSatDelta: -1 },
  },
  'event:evt_exp_esp_enemy_infiltration:targeted_counter_ops': {
    featureId: 'feature_counter_espionage_program',
    title: 'Counter-Espionage Program',
    description: 'Standing counter-intel cells run quiet sweeps without collapsing daily life.',
    category: 'military',
    ongoingEffect: { espionageNetworkDelta: 1 },
  },
  'event:evt_exp_esp_military_secrets_stolen:change_all_plans': {
    featureId: 'feature_operational_security',
    title: 'Operational Security Doctrine',
    description: 'A standing doctrine of compartmentalized planning blunts future intelligence leaks.',
    category: 'military',
    ongoingEffect: { militaryReadinessDelta: 1 },
  },
  'event:evt_exp_esp_underground_resistance:address_grievances': {
    featureId: 'feature_reform_commitment',
    title: 'Public Reform Commitment',
    description: 'The crown\'s visible concessions continue to mollify quiet dissenters turn after turn.',
    category: 'infrastructure',
    ongoingEffect: { commonerSatDelta: 1, nobilitySatDelta: -1 },
  },
  'event:evt_exp_fod_imports_blocked:enforce_strict_rationing': {
    featureId: 'feature_wartime_rationing',
    title: 'Wartime Rationing',
    description: 'Bread-and-measure tables in every market district stretch stores until the blockade breaks.',
    category: 'infrastructure',
    ongoingEffect: { foodDelta: 1, commonerSatDelta: -1 },
  },
  'event:evt_exp_kno_library_fire:let_it_burn': {
    featureId: 'feature_lost_library',
    title: 'Lost Library',
    description: 'The ash settles on what was. Each turn another scholar leaves for a court that still keeps books.',
    category: 'cultural',
    ongoingEffect: { culturalCohesionDelta: -1, clergySatDelta: -1 },
  },
  'event:evt_exp_mil_siege_preparations:full_siege_mobilization': {
    featureId: 'feature_siege_readiness',
    title: 'Siege Readiness',
    description: 'Walls are manned, stores stockpiled, engines drilled. The realm is ready for a long investment.',
    category: 'military',
    ongoingEffect: { militaryReadinessDelta: 1, treasuryDelta: -2 },
  },
  'event:evt_exp_mil_war_preparations:full_war_mobilization': {
    featureId: 'feature_war_footing',
    title: 'War Footing',
    description: 'Every smithy, every granary, every levy is turned to the coming campaign.',
    category: 'military',
    ongoingEffect: { militaryReadinessDelta: 1, treasuryDelta: -3 },
  },
  'event:evt_exp_po_refugee_influx:close_the_borders': {
    featureId: 'feature_closed_borders',
    title: 'Closed Borders',
    description: 'Papers required, gates watched. The realm is tight and so is trade.',
    category: 'infrastructure',
    ongoingEffect: { stabilityDelta: 1, merchantSatDelta: -1 },
  },
  'event:evt_exp_rel_sacred_desecration:call_for_reconciliation': {
    featureId: 'feature_faith_reconciliation',
    title: 'Faith Reconciliation Rite',
    description: 'A standing rite of cleansing keeps the sacred sites whole and the congregation unified.',
    category: 'cultural',
    ongoingEffect: { faithDelta: 1, culturalCohesionDelta: 1 },
  },
  'event:evt_exp_rel_sacred_desecration:hunt_perpetrators': {
    featureId: 'feature_purity_enforcement',
    title: 'Purity Enforcement Office',
    description: 'A permanent inquisitorial office hunts desecrators without waiting for another outrage.',
    category: 'cultural',
    ongoingEffect: { heterodoxyDelta: -1, commonerSatDelta: -1 },
  },
  'event:evt_exp_w2_plague_variant:open_hospitals_for_all': {
    featureId: 'feature_royal_hospitals',
    title: 'Royal Hospitals',
    description: 'Charter-funded infirmaries treat plague and pox through every season after.',
    category: 'cultural',
    ongoingEffect: { commonerSatDelta: 1, treasuryDelta: -1 },
  },
  'event:evt_exp_w2_plague_variant:pray_and_wait': {
    featureId: 'feature_providential_devotion',
    title: 'Providential Devotion',
    description: 'The faithful remember who led prayer while physicians faltered. The cathedral\'s authority endures.',
    category: 'cultural',
    ongoingEffect: { faithDelta: 1, heterodoxyDelta: -1 },
  },
  'event:evt_commoner_plague_outbreak:distribute_herbal_remedies': {
    featureId: 'feature_physicians_guild',
    title: 'Physicians\' Guild',
    description: 'A chartered guild of herbalists and physicians outlasts the outbreak that founded it.',
    category: 'cultural',
    ongoingEffect: { commonerSatDelta: 1, clergySatDelta: -1 },
  },
};
